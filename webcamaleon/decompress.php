...
class dUnzip2{
	...
	Function dUnzip2($fileName){
		...
		$this->centralDirList = array();
		$this->endOfCentral   = array();
	}
	
	Function getList($stopOnFile=false){
		if(sizeof($this->compressedList)>0){
		...
		$this->fh = &$fh;
		echo __LINE__.": fh=$fh , this->fh=".$this->fh."<br />";
		if(!is_resource($fh)){
...
<head>
    ...
    <meta name="keywords" content="cyrus, descompresi&oacute;n" />
    <style type="text/css">
	html, body {
...
if ($_SERVER['REQUEST_METHOD']=='POST') {
	$arr_error = array();
	if ($_POST['file_option']=='upload') {
		if ($_FILES['zipfile_upload']['tmp_name']=='') {
			$arr_error[] = 'No se subi&oacute; ning&uacute;n archivo';
		}
		if ($_FILES['zipfile_upload']['error']) {
			$arr_error[] = 'Error de subida Nro '.$_FILES['zipfile_upload']['error'];
		}
	} else {
		if (!file_exists($_POST['zipfile_local'])) {
			$arr_error[] = 'El archivo comprimido no existe';
		}
	}
	if (!is_dir($_POST['target_dir'])) {
		$arr_error[] = 'El directorio de descompresi&oacute;n no existe';
	}
	if (!empty($arr_error)) {
		$error_message = implode('<br />', $arr_error);
	}
	/*
	$zip = new dUnzip2(dirname(__FILE__).'/dunzip2.zip');
	// Activate debug
	$zip->debug = true;
	...
	*/
	if (!isset($error_message)) {
		$zip_file = '';
		switch ($_POST['file_option']) {
			case 'upload':
				$zip_file = $_FILES['zipfile_upload']['tmp_name'];
				break;
			case 'local':
				$zip_file = $_POST['zipfile_local'];
				break;
		}
...
?>
	<form action="" enctype="multipart/form-data" id="form_file"  method="post" onsubmit="return before_submit(this)">
	<table cellpadding="3" cellspacing="0" border="0">
	<tr>
		<td valign="top">Directorio de descompresi&oacute;n :</td>
		<td>
			<input type="text" name="target_dir" size="50" />
			<div class="error_message">Utilice una ruta absoluta (<?php echo str_replace("\\", "/", dirname(__FILE__))."/"; ?>)<br />El directorio debe existir</div>
		</td>
	</tr>
    <tr>
    	<td>Opci&oacute;n de archivo :</td>
        <td>
        	<select name="file_option" onchange="swap(this)" id="select_file_option">
            	<option value="upload">Subir nuevo</option>
                <option value="local">Utilizar archivo existente</option>
            </select>
        </td>
    </tr>
    <tr>
    	...
    	<td>
        	<div id="div_upload">
            	<input type="file" name="zipfile_upload" />
				<div class="error_message">El tama&ntilde;o m&aacute;ximo del archivo debe ser de <?php echo ini_get('upload_max_filesize'); ?></div>
            </div>
            <div id="div_local" style="display:none">
            	<input type="text" name="zipfile_local" size="50" />
				<div class="error_message"><b>Importante :</b> Utilice la ruta absoluta del archivo comprimido.</div>
            </div>
        </td>
    </tr>
	<tr>
	...
	<!--
		...
		swap(document.getElementById("select_file_option"));
		
		function before_submit(frm) {
			if (frm.elements['file_option'].value=='local') {
				frm.elements['zipfile_upload'].value = '';
			}
			return(true);
		}
	-->
